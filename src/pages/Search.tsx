...
const SearchPage: React.FC<Props> = ({ setView, formatPrice, products, categories, addToCart, userBalance = 0, onPurchase, onProductSelect }) => {
  ...
  let filteredProducts = products.filter((product) => {
    ...
    if (selectedPrice === 'under_10') {
      matchesPrice = getMinProductPrice(product) < 10;
    } else if (selectedPrice === '10_50') {
      matchesPrice = getMinProductPrice(product) >= 10 && getMinProductPrice(product) <= 50;
    } else if (selectedPrice === 'over_50') {
      matchesPrice = getMinProductPrice(product) > 50;
    }
             ...
             <div className="grid grid-cols-3 gap-3">
                {filteredProducts.map((product) => (
                    <ProductCard 
                        key={product.id} 
                        product={product} 
                        formattedPrice={formatPrice(getMinProductPrice(product))}
