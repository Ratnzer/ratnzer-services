...
import { getMinProductPrice } from '../utils/pricing';
...
const getOrderDate = (o: any) => {
  ...
  const [prodForm, setProdForm] = useState<Partial<Product>>({
    name: '', category: 'games', tag: '', imageColor: 'from-gray-700 to-gray-900', imageUrl: '', description: '',
  ...
  const handleSaveProduct = async () => {
    ...
    // ✅ Base product price has been removed entirely.
    // The app uses denominations (variants) for all pricing.
    // We only save product meta + regions/denominations/apiConfig/customInput.

    const productToSave: any = editingProduct
      ? ({ ...editingProduct, ...prodForm } as any)
      : {
    ...
    // Ensure we never send `price` to the backend
    if ('price' in productToSave) delete productToSave.price;
                 ...
                 <div key={p.id} className="bg-[#242636] p-3 rounded-xl flex items-center gap-3 border border-gray-700 hover:border-gray-500 transition-colors relative">
                    ...
                    <div className="flex-1">
                        <div className="flex justify-between items-start">
                            ...
                            <span className="text-xs font-mono font-bold text-yellow-400">${getMinProductPrice(p)}</span>
                        </div>
