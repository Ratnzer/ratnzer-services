    ...
    console.error('UI crashed:', error, info);
		  ...
		  render() {
		    if (this.state.hasError) {
		      return (
		        <div className="min-h-screen bg-[#13141f] flex flex-col items-center justify-center px-6 text-center animate-fadeIn">
		          <div className="text-3xl font-bold text-white mb-4">حدث خطأ مؤقت</div>
		          <div className="text-gray-400 mb-10 text-lg leading-relaxed max-w-sm">
		            حاولت فتح الصفحة بسرعة قبل اكتمال تحميل البيانات، اضغط رجوع أو أعد المحاولة.
		          </div>
		          <div className="flex flex-row-reverse w-full gap-4 max-w-md">
		            <button
		              onClick={() => {
		                this.setState({ hasError: false, error: null });
		                this.props.onReset?.();
		              }}
		              className="flex-1 py-4 rounded-xl bg-emerald-600 text-white font-bold active:scale-95 transition-all"
		            >
		              العودة للرئيسية
		            </button>
		            <button
		              onClick={() => window.location.reload()}
		              className="flex-1 py-4 rounded-xl bg-[#242636] text-gray-300 font-bold border border-gray-700 active:scale-95 transition-all"
		            >
		              إعادة المحاولة
		            </button>
		          </div>
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (currentView === View.ORDERS) {
      loadMyOrdersPage('silent'); // Direct refresh on entry
    }
    ...
    if (currentView === View.CART) {
      refreshCartFromServer('silent'); // Direct refresh on entry
    }

    if (currentView === View.NOTIFICATIONS) {
      refreshAnnouncementsFromServer('silent'); // Direct refresh on entry
    }
    ...
    <div className="flex justify-center min-h-screen bg-[#000000]">
      <div className="w-full bg-[#13141f] text-white font-cairo shadow-2xl relative flex flex-col min-h-screen">
        ...
        {currentView !== View.ADMIN && !isUserBanned && (
          <BottomNav currentView={currentView} setView={handleSetView} isHidden={isTermsOpen || isPrivacyOpen || isOtherModalOpen || isAddBalanceOpen || currentView === View.NOTIFICATIONS || currentView === View.WALLET || currentView === View.SEARCH || currentView === View.ABOUT_US || (currentView === View.ORDERS && isOrdersFromProfile)} />
        )}
